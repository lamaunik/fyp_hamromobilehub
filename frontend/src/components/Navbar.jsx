    ...
    <nav style={{ position:"fixed",top:0,left:0,right:0,zIndex:50,background:"rgba(255,255,255,0.97)",backdropFilter:"blur(12px)",borderBottom:`1px solid ${P.mist}`,boxShadow:"0 2px 16px rgba(0,27,72,0.07)",fontFamily:P.font }}>
      <div style={{ maxWidth:1280,margin:"0 auto",padding:"0 24px",height:64,display:"flex",alignItems:"center",justifyContent:"space-between" }}>
        ...
        <div style={{ display:"flex",alignItems:"center",gap:32 }}>
          {["Home","Products","Vendors","About","Contact"].map(item => (
            <a key={item} href="#" style={{ fontSize:14,fontWeight:600,color:P.royal,textDecoration:"none",transition:"color 0.2s" }}
      ...
      {menuOpen && (
        <div style={{ background:P.white,borderTop:`1px solid ${P.mist}`,padding:"16px 24px",display:"flex",flexDirection:"column",gap:14 }}>
          ...
          <Link to="/signin" style={{ fontSize:14,fontWeight:600,color:P.navy,textDecoration:"none" }}>Sign In</Link>
          <Link to="/signup" style={{ fontSize:14,fontWeight:700,color:P.white,background:`linear-gradient(135deg,${P.royal},${P.ocean})`,textDecoration:"none",padding:"10px 22px",borderRadius:999,textAlign:"center" }}>Get Started</Link>
        </div>
      )}
    </nav>
  );
