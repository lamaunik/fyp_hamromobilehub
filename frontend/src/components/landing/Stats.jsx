  ...
  return (
    <section style={{ background:P.white,padding:"0 24px 40px",fontFamily:P.font }}>
      <div style={{ maxWidth:1280,margin:"0 auto" }}>
        <div style={{ background:`linear-gradient(135deg,${P.navy} 0%,${P.royal} 50%,${P.ocean} 100%)`,borderRadius:24,padding:"56px 48px",position:"relative",overflow:"hidden",boxShadow:"0 16px 56px rgba(40, 43, 74, 0.25)" }}>
          ...
          <div style={{ position:"absolute",top:0,right:0,width:256,height:256,borderRadius:"50%",background:"rgba(212, 210, 195, 0.1)",filter:"blur(40px)" }}/>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:32,position:"relative",textAlign:"center" }}>
            {stats.map((s,i)=>(
              <div key={i}>
                ...
                <div style={{ color:"rgba(229, 227, 213, 0.75)",fontSize:14,fontWeight:600 }}>{s.label}</div>
              </div>
            ))}
          </div>

          <p style={{ position:"relative",textAlign:"center",color:"rgba(229, 227, 213, 0.65)",fontSize:13,marginTop:40 }}>
